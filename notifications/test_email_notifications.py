...
class EmailServiceTestCase(TestCase):
    ...
    def test_send_email_basic(self):
        ...
        self.assertIn('recipient@example.com', outbox[0].to)
    ...
    def test_send_email_html_template(self):
        ...
        self.assertTrue(success)
        self.assertEqual(len(outbox), 1)
        self.assertEqual(outbox[0].subject, 'HTML Test')
    ...
    def test_send_email_multiple_recipients(self):
        ...
        self.assertTrue(success)
        self.assertEqual(len(outbox), 1)
        self.assertEqual(set(outbox[0].to), set(recipients))
...
class MessageEmailSignalsTestCase(TestCase):
    ...
    def test_new_message_signal(self):
        ...
        self.assertEqual(len(outbox), 1)
        self.assertIn(self.doctor.email, outbox[0].to)
        self.assertIn('message', outbox[0].subject.lower())
    ...
    def test_message_from_doctor(self):
        ...
        self.assertEqual(len(outbox), 1)
        self.assertIn(self.patient.email, outbox[0].to)
...
class VideoCallEmailSignalsTestCase(TestCase):
    ...
    def test_incoming_call_signal(self):
        ...
        self.assertEqual(len(outbox), 1)
        self.assertIn(self.doctor.email, outbox[0].to)
        self.assertIn('call', outbox[0].subject.lower())
    ...
    def test_call_status_ongoing_signal(self):
        ...
        self.assertEqual(len(outbox), 2)
        emails_to = {email.to[0] for email in outbox}
